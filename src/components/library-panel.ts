    ...
    return html`
      ...
      <div class="h-full bg-[#f4e4bc] border-r border-[#d3c29d] shadow-[10px_0_30px_rgba(0,0,0,0.3)] transition-transform duration-500 ease-in-out pointer-events-auto overflow-hidden relative"
        ...
        <div class="relative p-6 h-full flex flex-col items-center">
           <h2 class="font-serif text-xl font-black text-[#5c4b37] opacity-60 uppercase tracking-[0.3em] mb-4">Music Library</h2>
           <div class="w-full h-px bg-[#5c4b37] opacity-10 mb-4"></div>
           <p class="text-[#5c4b37]/40 font-serif italic text-base">The complete catalog awaits your selection...</p>
        </div>
        
        <!-- Close Button for Library -->
        <button @click=${this.handleClose} class="absolute top-4 right-4 text-[#5c4b37] opacity-40 hover:opacity-100 transition-opacity p-2 cursor-pointer">
